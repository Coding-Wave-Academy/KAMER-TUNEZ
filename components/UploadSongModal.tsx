...
const UploadSongModal: React.FC<UploadSongModalProps> = ({ onClose }) => {
    ...
    const handleFileSelect = (file: File) => {
        ...
        if (!ALLOWED_FORMATS.includes(file.type)) {
            setError(`Invalid file format. Please use MP3, WAV, or OGG.`);
            return;
        }
        // Validate size
        ...
        audio.onloadedmetadata = () => {
            if (audio.duration > MAX_DURATION_SECONDS) {
                setError(`Song is too long. Max duration is ${MAX_DURATION_SECONDS / 60} minutes.`);
